...
--coffeeshop_id temp set to NULL 
...
--city_id temp set to NULL
INSERT INTO shops VALUES(1, 'Common Grounds', NULL);

--set coffeeshop_id for first two employees
UPDATE employees
SET coffeeshop_id = 1
...
--insert first two values where coffeeshop_id = 1 into suppliers table
...
--employees who work in common grounds coffeeshop
SELECT *
FROM employees
WHERE coffeeshop_id = 1;
...
--supplier is beans and barley from supplier table
SELECT *
FROM suppliers
WHERE supplier_name = 'Beans and Barley';

--supplier that are not beans and barley from supplier table
SELECT *
FROM suppliers
...
--coffee types that are not robusta or arabica
SELECT *
FROM suppliers
...
--employees whose emails are not missing
SELECT *
FROM employees
...
--employees making less than 50k, then true, otherwise false
SELECT
	first_name || ' ' || last_name AS full_name,
	salary,
...
--female employees making less than 50k, then true, otherwise false
SELECT
	first_name || ' ' || last_name AS full_name,
	salary,
	gender,
...
--substring and position to find employee email
SELECT
	email,
	SUBSTRING(email FROM POSITION ('@' IN email) +1)
FROM employees;

--fill missing emails with custom value
SELECT
	email,
...
--total salaries for each coffeeshop
SELECT coffeeshop_id,
	SUM(salary)
FROM employees
GROUP BY coffeeshop_id;

--number of employees,avg, min, max, total salaries for each coffeeshop
SELECT
	coffeeshop_id,
	COUNT(*) AS num_of_emp,
...
--coffeeshops with more than 200 employees
SELECT
	coffeeshop_id,
	COUNT(*) AS num_of_emp,
	ROUND(AVG(salary), 0) AS avg_sal,
...
--coffeeshops with a minimum salary of less than 10k
SELECT
	coffeeshop_id,
	COUNT(*) AS num_of_emp,
	ROUND(AVG(salary), 0) AS avg_sal,
...
--count of employees in each pay category
...
--subquery employees who work in US coffee shops
SELECT *
FROM employees
...
--subquery employees who makes over 35k and work in US coffee shops
SELECT *
FROM employees
WHERE salary > 35000
